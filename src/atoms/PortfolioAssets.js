...
export const allPortfolioBoughtAssetsFromAPI = selector({
  ...
  get: async ({get}) => {
    const portfolioBoughtAssets = get(allPortfolioBoughtAssetsInStorage)
    const portfolioAssetsMarketData = 
      await getWatchlistedData(1, portfolioBoughtAssets.map((portfolioAsset) => portfolioAsset.id).join(','))
    
    const boughtAssets = portfolioBoughtAssets.map((boughtAsset) => {
